      ...
      chunks.forEach(async (chunk, index) => {
        ...
        if (attach) {
          ndkEvent.tags.push([
            "e",
            attach,
            ndk?.explicitRelayUrls ? ndk?.explicitRelayUrls[0] : "",
            "root"
          ])
        }
        if (tags) {
...
const chunkPayload = (base64, customChunkSize) => {
 ...
 *
 */
export const reassemble = events => {
  ...
  for (const group of Object.values(groups)) {
    group.sort(
      (a, b) =>
        parseInt(a.tags.find(getTag("index"))[1]) -
        parseInt(b.tags.find(getTag("index"))[1])
    )
  }
